"""Offline identity and fail-closed checks for the ratified Together r4 gate."""
...
from paceprompt_eval import issue145_11model_together_gate as gate_module
from paceprompt_eval.issue145_11model_together_gate import (
...
from paceprompt_eval.issue145_11model_together_run import (
...
class TogetherR4GateTests(unittest.TestCase):
    def setUp(self) -> None:
        old = safe_run_dir("issue145-11model-together-profile-r4-20260925-01", create=False)
    ...
    def test_ratification_queue_requests_and_conservative_bound(self) -> None:
        ...
        minimax = next(spec for spec in specs
                       if spec.requested_model_id == "minimax/minimax-m3")
        self.assertEqual(minimax.provider_endpoint, "together")
        self.assertIsNone(minimax.quantization)
    ...
    def test_external_scorer_or_contract_change_fails_before_credential(self) -> None:
        ...
        from paceprompt_eval.issue145_11model_together_gate import sha256_file
        ...
        with patch("paceprompt_eval.issue145_11model_together_gate.sha256_file", side_effect=changed):
        ...
        with patch("paceprompt_eval.issue145_11model_together_gate.sha256_file",
...
class TogetherR4LiveBoundaryTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_rejects_run_directory_symlink_alias(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            with patch("paceprompt_eval.issue145_11model_together_gate.RUNS_ROOT", root):
    ...
    async def test_offline_gate_prepares_and_seals_with_exact_request_mapping(self) -> None:
        ...
        with TemporaryDirectory() as temporary:
            directory = Path(temporary)
            with (patch("paceprompt_eval.issue145_11model_together_gate.checked_run_dir",
                        return_value=directory),
                  patch("paceprompt_eval.issue145_11model_together_gate.ratified_profile",
                        return_value=(profile, queue))):
                gate = await prepare_gate("issue145-11model-together-matrix-20260925-01", fetch=fetch)
                ...
                sealed = await asyncio.to_thread(
                    seal_gate, "issue145-11model-together-matrix-20260925-01"
                )
                self.assertEqual(sealed["status"], "awaitingFinalLiveRunAuthorization")
                checked = await asyncio.to_thread(
                    verify_sealed_gate, "issue145-11model-together-matrix-20260925-01"
                )
                self.assertEqual(checked["status"], "valid")
                repair_seal = {"path": "Evaluation/WorkoutImport/SourceRepairBridges/review.json",
                               "sha256": "a" * 64}
                with (patch("paceprompt_eval.issue145_11model_together_gate.host_source_tree_hash",
                            return_value="b" * 64),
                      patch("paceprompt_eval.issue145_11model_together_gate.verify_source_chain")
                      as bridge):
                ...
                model = profile["candidateModelIDs"][0]
                payload_path = directory / "mock-payloads" / f"{model.replace('/', '--')}.json"
                payload = strict_json_load(payload_path)
                payload["body"]["messages"][0]["content"] = "Changed system prompt"
                write_json(payload_path, payload)
                summary_path = directory / "mock-summary.json"
                summary = strict_json_load(summary_path)
                summary["payloadHashes"][model] = gate_module.sha256_file(payload_path)
                write_json(summary_path, summary)
                with self.assertRaisesRegex(RuntimeError, "mock request diverges from replay"):
                    await asyncio.to_thread(
                        verify_sealed_gate,
                        "issue145-11model-together-matrix-20260925-01",
                    )
    ...
    async def test_wrong_authorization_cannot_read_key_or_fetch_public_catalogue(self) -> None:
        ...
        with patch(
            "paceprompt_eval.issue145_11model_together_run.verify_sealed_gate",
            return_value={"authorizationPhrase": "EXACT_PHRASE"},
        ):
            with self.assertRaisesRegex(RuntimeError, "exact reviewed"):
                await run_live(
                    run_id="issue145-11model-together-matrix-20260925-01",
