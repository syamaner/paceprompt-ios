"""Zero-spend, source-bound gate for the ratified issue #145 Together r4 matrix.
...
from .issue145_11model_together_r4 import (
    PROPOSAL_RUN_ID, PROPOSED_ROOT_RUN_ID,
    materialized_models_r4 as materialized_models,
    profile_material_r4 as profile_material,
)
...
RATIFICATION = HOST_EVAL_ROOT / "issue145-11model-together-r4-ratification-r1.json"
PROFILE_SHA256 = "913620ebed0f69398d9530630ecd2ee4ba110b534cc3e554ea73b5b6210d968c"
PROFILE_EVIDENCE_SHA256 = "5329b255e0c6b5781f265b52ba703ec68447f64d0efef7b628daf77c1c7269d0"
PROFILE_SOURCE_SHA256 = "10db3fcb72384275a97675545aaa0720e0629263533914133952fb42e74d1dc8"
HARD_LIMIT_USD = "300.00"
GATE_VERSION = "paceprompt-host-eval-operator-gate/issue145-11model-together-r4-r1"
CHILD_GATE_VERSION = "paceprompt-host-eval-operator-gate/issue145-11model-together-child-r1"
AUTH_PREFIX = "AUTHORIZE_PACEPROMPT_ISSUE145_11MODEL_TOGETHER_"
...
def _replay_saved_evidence(
    directory: Path, specs: tuple[ModelSpec, ...], *, replay_mocks: bool,
) -> tuple[dict[str, Any], dict[str, dict[str, Any]] | None]:
    """Derive route, price and optional outbound mocks from raw saved bytes."""
    saved_catalogue = directory / "catalogue"
    raw_by_url = {MODELS_URL: saved_catalogue / "models.json"}
    for spec in specs:
        if spec.canonical_revision is None:
            raise RuntimeError("ratified route has no canonical revision")
        raw_by_url[endpoint_url(spec.canonical_revision)] = (
            saved_catalogue / f"{spec.requested_model_id.replace('/', '--')}.json"
        )

    def saved_fetch(url: str) -> bytes:
        if url not in raw_by_url:
            raise RuntimeError("saved public catalogue URL is not pinned")
        return raw_by_url[url].read_bytes()

    with TemporaryDirectory(prefix="paceprompt-together-gate-replay-") as temporary:
        ...
        if selected != strict_json_load(saved_catalogue / "selected.json"):
            raise RuntimeError("saved route or price diverges from raw catalogue")
        if any(type(item.get("status")) is not int or item["status"] != 0
               for item in selected["selected"]):
            raise RuntimeError("saved public route is unavailable")
        if not replay_mocks:
            return selected, None
        _, templates = asyncio.run(mock_payloads(
            replay_dir, selected,
            models_path=directory / "models-11-materialized.json",
        ))
        saved_summary = strict_json_load(directory / "mock-summary.json")
        replay_summary = strict_json_load(replay_dir / "mock-summary.json")
        if ({key: value for key, value in saved_summary.items()
             if key != "payloadHashes"}
                != {key: value for key, value in replay_summary.items()
                    if key != "payloadHashes"}):
            raise RuntimeError("saved mock summary diverges from replay")
        for spec in specs:
            name = f"{spec.requested_model_id.replace('/', '--')}.json"
            saved_path = directory / "mock-payloads" / name
            saved = strict_json_load(saved_path)
            replayed = strict_json_load(replay_dir / "mock-payloads" / name)
            if saved_summary["payloadHashes"].get(spec.requested_model_id) != sha256_file(
                saved_path
            ):
                raise RuntimeError("saved mock payload hash changed")
            # Inspect generates a random x-irid per capture; every other
            # request field, including the full prompt body, must replay.
            for payload in (saved, replayed):
                payload["headers"].pop("x-irid", None)
            if saved != replayed:
                raise RuntimeError("saved mock request diverges from replay")
        return selected, templates


def _expected_ratification() -> dict[str, Any]:
    return {
        "ratificationVersion": "paceprompt-host-eval-ratification/issue145-11model-together-r4-r1",
        "status": "profile-and-cap-ratified-live-run-not-authorized",
        "source": "operator confirmed the exact Together r4 profile and USD 300 cap in the 2026-09-25 Codex conversation",
...
def model_specs(profile: dict[str, Any]) -> tuple[ModelSpec, ...]:
    materialized = materialized_models(profile)
    by_id = {item["requestedModelID"]: item for item in materialized["models"]}
    if list(by_id) != profile["candidateModelIDs"]:
...
async def prepare_gate(
    ...
    models = materialized_models(profile)
...
def _expected_gate(run_dir: Path, *, source_hash: str | None = None) -> dict[str, Any]:
    ...
    expected_models = materialized_models(profile)
    ...
    selected, templates = _replay_saved_evidence(
        run_dir, model_specs(profile), replay_mocks=True,
    )
...
def _child_material(
    ...
    profile, queue = ratified_profile()
    selected, _ = _replay_saved_evidence(
        directory, model_specs(profile), replay_mocks=False,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Zero-spend issue #145 Together r4 gate")
