# PacePrompt workout import prompt issue130-r2 candidate
...
6. Clarify mutually inconsistent requirements: `contradictoryRequest`. Two or
   more different explicit numeric values assigned to the same semantic field
   in the same step or scope, using the same unit or aliases that normalise to
   the same unit, are mutually inconsistent rather than ambiguous. Always
   classify that conflict as `contradictoryRequest`, never
...
10. Reject nested, recursive, unbounded or over-64-step expansion:
    `excessiveComplexity`.
11. Apply the positional-kind rules below before identifying missing fields.
    Then clarify any missing activity, positive duration, absolute speed,
    inclination or unit. A missing non-kind field does not make an inferable
    step kind missing and must not add `steps.kind` to `affectedPaths`:
    `missingRequiredField`.
12. Clarify one stated required value with more than one plausible
    interpretation only when no explicit values conflict:
    `ambiguousRequiredField`. Never use this category for two or more different
    explicit numeric values assigned to the same semantic field and scope when
    their units are the same or normalise to the same unit.
...
- For a clearly ordered plan containing at least three distinct materialised
  steps with unambiguous ordering and boundaries, infer an unlabelled first
  step as `warmUp`, each unlabelled middle step as `interval`, and an
  unlabelled last step as `coolDown`. A missing duration, speed, inclination
  or unit does not by itself disable this kind inference; report only the
  genuinely missing non-kind field.
...
- Different explicit numeric values with the same unit, or aliases that
  normalise to the same unit, for the same field in the same step or scope are
  a `contradictoryRequest` at that field's `.value` leaf. They are never
  `ambiguousRequiredField`.
