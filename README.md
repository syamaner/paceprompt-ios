...
- a deliberate saved-plan JSON export flow that requires record selection, previews the exact filename, category, count and included fields before sharing, writes only decoded supported records to protected temporary storage, and removes the temporary copy when sharing finishes or is cancelled;
- a deliberate workout-history JSON export flow for selected supported version-2 summaries, with a checked agent-readable prescribed-versus-executed fixture, exact numeric units, visible eligibility failures, protected temporary storage and cleanup after the system share sheet;
...
The production app does **not** contain plan recovery, recovery-byte export, complete-store export, History deletion UI, bulk deletion or local-data reset UI, automatic or background export or inference, Apple Foundation Models integration, provider fallback, model comparison results, an FTMS Start/Stop/Pause command, HealthKit reads, automatic Health saving or a watchOS app. The developer-only evaluation target, corpus, scorer and raw-run boundary remain excluded from the production bundle. The production OpenRouter path sends nothing until the user reviews the exact disclosure and agrees to that one request.
...
- `WorkoutHistoryExportMapper` materialises only supported, internally consistent version-2 summaries into the checked JSON schema without exporting plan prose, Health receipts, raw telemetry, command evidence or device identity. `WorkoutHistoryExporter` owns the protected temporary artifact; `HistoryLibraryViewModel` keeps record selection, exact preview, share preparation, cleanup and persistent History separate.
...
- The local storage contract keeps saved plans, execution summaries and deliberately ephemeral exports separate. History presents supported local records, deliberate Apple Health state and selected version-2 JSON export; reset, recovery UI, recovery-byte export, complete-store export and History deletion remain deferred.
