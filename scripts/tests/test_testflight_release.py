...
class ReleaseGuardTests(unittest.TestCase):
    ...
    def test_current_source_matches_fresh_tag(self):
        self.assertEqual(guard.check_tag("testflight/1.0.1-b8", PROJECT), ("1.0.1", "8"))

    def test_rejects_wrong_tag_or_build(self):
        for tag in ("testflight/1.0.1-b7", "testflight/1.1-b8", "testflight/1.0.1-b08",
                    "testflight/1.0.1-b0", "testflight/1.0.1-b8/extra", "release/1.0.1-b8"):
            with self.subTest(tag=tag), self.assertRaises(ValueError):
                guard.check_tag(tag, PROJECT)

    def test_rejects_release_debug_mismatch_and_bundle_change(self):
        with self.assertRaises(ValueError):
            guard.check_tag("testflight/1.0.1-b8", PROJECT.replace("CURRENT_PROJECT_VERSION = 8;", "CURRENT_PROJECT_VERSION = 7;", 1))
        with self.assertRaises(ValueError):
            guard.check_tag("testflight/1.0.1-b8", PROJECT.replace("PRODUCT_BUNDLE_IDENTIFIER = com.otherweather.PromptPace;", "PRODUCT_BUNDLE_IDENTIFIER = other.app;", 1))
    ...
    def test_rejects_missing_purpose_and_changed_version(self):
        ...
        guard.metadata(info, "1.0.1", "8")
        for invalid in ("NO", 0, True):
            with self.subTest(encryption=invalid), self.assertRaises(ValueError):
                guard.metadata({**info, "ITSAppUsesNonExemptEncryption": invalid}, "1.0.1", "8")
        for key in info:
            changed = dict(info)
            del changed[key]
            with self.subTest(key=key), self.assertRaises(ValueError):
                guard.metadata(changed, "1.0.1", "8")
    ...
    def test_source_rejects_missing_exact_head_review(self):
        ...
        def fake_git(*args):
            ...
            if args[0] == "rev-parse":
                return sha
            if args[0] == "ls-remote":
                return f"{sha}\trefs/tags/testflight/1.0.1-b8"
            if args[0] == "fetch":
                return ""
        ...
        with patch.object(guard, "git", side_effect=fake_git), patch.object(guard, "api", return_value=[pull]):
            with self.assertRaisesRegex(ValueError, "attestation"):
                guard.source("testflight/1.0.1-b8", sha)
