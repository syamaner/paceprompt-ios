...
- For a compatible, developer-only issue #145 repair confined to `Evaluation/WorkoutImport/HostEval/`, apply `Evaluation/WorkoutImport/HostEval/issue145-standing-restart-and-repair-r1.md`: run the affected focused HostEval, corpus, schema, scorer and static checks, inspect the complete diff, run `git diff --check`, and obtain independent review. A complete iOS simulator/build gate is not required unless app, shared test/build, release, production, safety or repository validation inputs change, or the focused review identifies cross-boundary risk. This exception never authorises credential access or provider calls.
...
- Any later change to app code, app or shared tests, Xcode project/build settings, dependencies, validation scripts, executable resources or safety logic invalidates the gate and requires affected focused checks followed by one replacement complete gate. The narrow issue #145 HostEval-only exception above applies only while its compatibility invariants hold.
